...
- [x] 2.4 Fetch unique release-group artwork into a persistent cache, publish available covers as same-origin files, and confirm albums without art render the placeholder without a remote image URL.
...
- [x] 3.4 Restore and save the artwork cache in GitHub Actions, document cache behavior and local build setup, and confirm repeat builds skip cached successful and no-art lookups.
- [x] 3.5 Deploy the updated site and verify album images load from the site origin without runtime Cover Art Archive requests.
